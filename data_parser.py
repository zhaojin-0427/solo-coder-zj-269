...
def get_filter_options(df):
    if df is None or df.empty:
        return [], [], [], [], []
    months = [{'label': m, 'value': m} for m in sorted(df['月份'].unique(), reverse=True)]
    cats = [{'label': c, 'value': c} for c in sorted(df['类别'].unique())]
    pays = [{'label': p, 'value': p} for p in sorted(df['支付方式'].unique())]
    scenes = [{'label': s, 'value': s} for s in sorted(df['场景'].unique())]
    goals = [{'label': g, 'value': g} for g in sorted(df['消费目标'].unique())]
    return months, cats, pays, scenes, goals
